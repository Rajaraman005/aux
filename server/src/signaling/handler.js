  ...
  wss.on("connection", async (ws, req) => {
    ...
    ws.on("close", async () => {
      ...
      matchmaking.handleDisconnect(userId).catch((err) => {
        ...
        console.error("World video disconnect cleanup error:", err.message);
...
async function handleWorldLeave(userId) {
...
async function handleWorldNext(userId, message) {
  ...
  try {
    ...
    // End current session and re-queue the user
